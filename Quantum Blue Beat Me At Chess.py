...
def get_AI_move(board):
    ...
    for move in list(board.legal_moves):
        new_board = board.copy()
        new_board.push(move)
        score = alpha_beta(new_board, depth, alpha, beta, False) + Qfish_Evaluation(new_board) + VQC_Evaluation(new_board)
        if score > alpha:
            alpha = score
            best_move = move
    return best_move
...
def display_board(board):
    ...
    flipped_board = chess.svg.board(board, flipped=True)
...
print("AI's initial move:", ai_move)
...
while not board.is_game_over():
    ...
    if not board.turn:  # Black's turn (user)
